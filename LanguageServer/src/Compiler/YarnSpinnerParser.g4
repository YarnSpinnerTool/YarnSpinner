...
header 
    : header_key=HEADER_KEY HEADER_DELIMITER  header_value=REST_OF_LINE?
    ;
...
variable
    : VAR_ID
    | {NotifyErrorListeners("Invalid variable id syntax");} MALFORMED_VAR_ID
    | {NotifyErrorListeners("Variable Id's need to start with a $ symbol");} FUNC_ID
    ;
declared_variable
    : VAR_ID
    | {NotifyErrorListeners("Invalid variable id syntax");} MALFORMED_VAR_ID 
...
function_call 
    : function_name lparens='(' function_parameter? (COMMA function_parameter)* rparens=')' 
    | function_name lparens='(' function_parameter? (COMMA function_parameter)* ({NotifyErrorListeners("Argument missing");} COMMA)+ rparens=')'  
    | {NotifyErrorListeners(") expected");}function_name lparens='(' function_parameter? (COMMA function_parameter)* ({NotifyErrorListeners("Argument missing");} COMMA)+
    | {NotifyErrorListeners("( expected");}function_name 
    ;
...
call_statement
    ...
    | COMMAND_START {NotifyErrorListeners("Function call expected");} COMMAND_CALL  COMMAND_END
    ;

command_statement
    : COMMAND_START command_formatted_text COMMAND_TEXT_END (hashtag*)
    | {NotifyErrorListeners("Command text expected");} COMMAND_START {NotifyErrorListeners("Command text expected");} COMMAND_END (hashtag*)
    ;

command_formatted_text
	: command_name_rule command_parameter*
	;

command_parameter : (COMMAND_TEXT|COMMAND_EXPRESSION_START expression EXPRESSION_END);
...
declare_statement
    : COMMAND_START COMMAND_DECLARE declared_variable OPERATOR_ASSIGNMENT value ('as' type=FUNC_ID)? COMMAND_END ;

jump_statement
    : COMMAND_START COMMAND_JUMP jump_destination COMMAND_END
    ;
jump_destination: COMMAND_ID;
