This document talks about how to use \hyperlink{a00045}{Yarn} if you're using it to write content. It doesn't talk about how to integrate \hyperlink{a00045}{Yarn} Spinner into your project; for that, see \href{../YarnSpinner-Unity}{\tt \char`\"{}\-Using Yarn Spinner in your Unity game\char`\"{}}.
...
If there are no options when we reach the end of a node, then we've reached the end of a conversation, and \hyperlink{a00045}{Yarn} Spinner will let your game know.
...
When this is run, \hyperlink{a00045}{Yarn} Spinner will behave just as if you'd broken all of this up into multiple nodes.
...
\hyperlink{a00045}{Yarn} has a special command, called {\ttfamily stop}. If you include {\ttfamily $<$$<$stop$>$$>$} in your node, \hyperlink{a00045}{Yarn} will stop the entire conversation when it reaches it.
...
There are four different types of value in \hyperlink{a00045}{Yarn}\-: strings, numbers, booleans, and {\ttfamily null}.
...
\hyperlink{a00045}{Yarn} will automatically convert between types for you. For example\-: \begin{DoxyVerb}<<if "hi" == "hi">>
...
You can call functions in your {\ttfamily if} commands. For example, \hyperlink{a00045}{Yarn} Spinner provides a function called {\ttfamily visited}, which you can use to find out if a node has been entered before or not. It takes a single parameter (which is a string), and returns true or false depending on whether or not the node you specified has been entered. \begin{DoxyVerb}<<if visited("GoToCity")>>
    We have gone to the city before!
<<endif>>
\end{DoxyVerb}


You can't define your own functions inside \hyperlink{a00045}{Yarn} itself, but they can be added at run-\/time. See \hyperlink{a00210}{Extending.\-md}Extending \hyperlink{a00045}{Yarn} Spinner\char`\"{}\char`\"{} for more info. 