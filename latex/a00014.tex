This document talks about how to use \hyperlink{a00053}{Yarn} if you're using it to write content. It doesn't talk about how to integrate \hyperlink{a00053}{Yarn} Spinner into your project; for that, see \href{../YarnSpinner-Unity}{\tt \char`\"{}\-Using Yarn Spinner in your Unity game\char`\"{}}.

\subsection*{Lines}

\hyperlink{a00053}{Yarn} lets you define multiple chunks of dialogue that are linked together. Each chunk is called a {\itshape node}.
...
You don't have to provide the label. Instead, you can just provide the node name, like so\-: ``` \mbox{[}\mbox{[}Go\-To\-City\mbox{]}\mbox{]} ``` If an option has no label, \hyperlink{a00053}{Yarn} will automatically jump to that node when that line is run, and it won't show options to the player.
...
If there are no options when we reach the end of a node, then we've reached the end of a conversation, and \hyperlink{a00053}{Yarn} Spinner will let your game know.

\subsection*{Shortcut Options}

Sometimes, you'll want to add little branches to your conversation, but you don't want to create separate nodes for them. Instead, you can use {\itshape shortcut options}\-: ``` Mae\-: What did you say to her? -\/$>$ Nothing. Mae\-: Oh, man. Maybe you should have. -\/$>$ That she was an idiot. Mae\-: Hah! I bet that pissed her off. Mae\-: Anyway, I'd better get going. ``` When this is run, \hyperlink{a00053}{Yarn} Spinner will behave just as if you'd broken all of this up into multiple nodes.
...
\hyperlink{a00053}{Yarn} has a special command, called {\ttfamily stop}. If you include {\ttfamily $<$$<$stop$>$$>$} in your node, \hyperlink{a00053}{Yarn} will stop the entire conversation when it reaches it.
...
Maths is easy in \hyperlink{a00053}{Yarn}. ``` $<$$<$ set \$number\-\_\-of\-\_\-stars\-\_\-collected = 1 $>$$>$ $<$$<$ set \$number\-\_\-of\-\_\-stars\-\_\-collected = \$number\-\_\-of\-\_\-stars\-\_\-collected + 1 $>$$>$ ``` \begin{quotation}
...
There are four different types of value in \hyperlink{a00053}{Yarn}\-: strings, numbers, \href{http://eesemi.com/boolean.htm}{\tt booleans}, and {\ttfamily null}.
...
\hyperlink{a00053}{Yarn} will automatically convert between types for you. For example\-: ``` $<$$<$if \char`\"{}hi\char`\"{} == \char`\"{}hi\char`\"{}$>$$>$ The two strings are the same! $<$$<$endif$>$$>$

$<$$<$if 1+1+\char`\"{}hi\char`\"{} == \char`\"{}2hi\char`\"{}$>$$>$ Strings get joined together with other values! $<$$<$endif$>$$>$ ```

\subsection*{Functions}

You can call functions in your {\ttfamily if} commands. For example, \hyperlink{a00053}{Yarn} Spinner provides a function called {\ttfamily visited}, which you can use to find out if a node has been entered before or not. It takes a single parameter (which is a string), and returns true or false depending on whether or not the node you specified has been entered. ``` $<$$<$if visited(\char`\"{}\-Go\-To\-City\char`\"{})$>$$>$ We have gone to the city before! $<$$<$endif$>$$>$ ``` You can't define your own functions inside \hyperlink{a00053}{Yarn} itself, but they can be added at run-\/time. See ../\-Yarn\-Spinner-\/\-Programming/\-Extending.md \char`\"{}\char`\"{}Extending \hyperlink{a00053}{Yarn} Spinner\char`\"{}\char`\"{} for more info.
