This document talks about how to use \hyperlink{a00026}{Yarn} if you're using it to write content. It doesn't talk about how to integrate \hyperlink{a00026}{Yarn} Spinner into your project; for that, see \hyperlink{a00100}{Unity.\-md}Using \hyperlink{a00026}{Yarn} Spinner in your Unity game\char`\"{}\char`\"{}.

\subsection*{Lines}

\hyperlink{a00026}{Yarn} lets you define multiple chunks of dialogue that are linked together. Each chunk is called a {\itshape node}.
...
If an option has no label, \hyperlink{a00026}{Yarn} will automatically jump to that node when that line is run, and it won't show options to the player.
...
If there are no options when we reach the end of a node, then we've reached the end of a conversation, and \hyperlink{a00026}{Yarn} Spinner will let your game know.
...
When this is run, \hyperlink{a00026}{Yarn} Spinner will behave just as if you'd broken all of this up into multiple nodes.
...
\hyperlink{a00026}{Yarn} has a special command, called {\ttfamily stop}. If you include {\ttfamily $<$$<$stop$>$$>$} in your node, \hyperlink{a00026}{Yarn} will stop the entire conversation when it reaches it.
...
There are four different types of value in \hyperlink{a00026}{Yarn}\-: strings, numbers, booleans, and {\ttfamily null}.
...
\hyperlink{a00026}{Yarn} will automatically convert between types for you. For example\-: \begin{DoxyVerb}<<if "hi" == "hi">>
...
You can call functions in your {\ttfamily if} commands. For example, \hyperlink{a00026}{Yarn} Spinner provides a function called {\ttfamily visited}, which you can use to find out if a node has been entered before or not. It takes a single parameter (which is a string), and returns true or false depending on whether or not the node you specified has been entered. \begin{DoxyVerb}<<if visited("GoToCity")>>
    We have gone to the city before!
<<endif>>
\end{DoxyVerb}


You can't define your own functions inside \hyperlink{a00026}{Yarn} itself, but they can be added at run-\/time. See \hyperlink{a00097}{Extending.\-md}Extending \hyperlink{a00026}{Yarn} Spinner\char`\"{}\char`\"{} for more info. 