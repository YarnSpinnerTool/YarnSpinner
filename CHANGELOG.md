...
- Added a new method, `LineParser.ParseStringAndIncludeMarkupDiagnostics`.
  - This allows access to the diagnostics that are generated by parsing the markup.
- Added a new method, `Library.GenerateUniqueContentViewedVariableName`.
  - This returns the same unique value used to generate `once` variables.
  - This allows getting access to the underlying variable name either for debugging or resetting during development.
  - This uses the lineID of the once attributed line, so won't be stable without a lineID.
  - To reset a variable, you can do something like the following:
    ```
    title: Start
    ---
    I am the onced line <<once>> #line:abc123
    ===
    ```

    Later on in your game, you can access or modify this variable:
    
    ```
    // this resets the once variable
    runner.VariableStorage.SetValue(Yarn.Library.GenerateUniqueContentViewedVariableName("line:abc123"), false);
    ```
  - This works for once statements, with or without a variable associated with them.
- Added a new method, `Library.GenerateUniqueCommandBlockViewedVariableName()`.
  - This returns the same unique value to generate the `<<once>>` block variables.
  - This is inherently unstable, and should only be used either after editing of Yarn has finished or with an understanding of it's instability. This is something we want to improve in future releases.
...
- When tagging lines, you can now define what `ILineTagGenerator` you want to use.
  - This defaults to using `RandomLineTagGenerator` if you don't set one, which is the same behaviour as previous versions of Yarn Spinner.
...
- Fixed a compiler crash caused by invalid markup structures.
...
- Removed the obsolete `AddTagsToLines` call from `YarnSpinner.Compiler.Utility`.
  - Instead, you should now use the newer method `YarnSpinner.Compiler.Utility.TagLines`, which supports using custom `ILineTagGenerator`.
